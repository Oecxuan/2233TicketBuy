...
"""
import logging, os as _os, shutil as _shutil, sys as _sys, signal, threading
...
# ====== 彻底压制 BTB / cptoken 日志 ======
for _btb in ['cptoken', 'util.TimeUtil', 'util.request', 'util', 'ntplib',
             'urllib3', 'httpcore', 'httpx', 'h2', 'chardet', 'btb',
             'src.cptoken_vendor']:
    _l = logging.getLogger(_btb)
    _l.setLevel(logging.CRITICAL)
    _l.propagate = False
    _l.handlers.clear()

# 清理 btb*.log 文件 + btb_logs 目录
def _clean_btb_logs():
...
# 运行时持续抑制 cptoken 写入（周期性清理 + 环境变量）
_os.environ['BTB_LOG_LEVEL'] = 'CRITICAL'
_os.environ['CPTOKEN_LOG_LEVEL'] = 'CRITICAL'
# ======
...
def confirm_and_grab(config, api, viewers=None):
    ...
    result = grab_ticket_interactive(config, viewers=viewers or [], interrupt_event=_interrupt_event)
...
示例:
  ...
  python main.py --grab       # 直接抢票
    ...
    if args.grab:
        ...
        # 从缓存或 config 加载购票人
        viewers = _load_viewers_cache() or config.event.viewers
        if not viewers:
            print("\n未找到购票人信息，请先交互模式选择活动")
            return
