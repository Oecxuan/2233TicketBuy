...
class BilibiliAPI:
    ...
    # ========== Session persistence (BHYG-style) ==========
    
    @property
    def _session_file(self) -> str:
        import os
        uid = self.config.user.dede_user_id or "unknown"
        d = os.path.join(os.path.expanduser("~"), ".2233TicketBuy")
        os.makedirs(d, exist_ok=True)
        return os.path.join(d, f"session_{uid}.json")
    ...
    def load_session(self) -> bool:
        ...
        try:
            with open(self._session_file, "r", encoding="utf-8") as f:
                cookies = json.load(f)
            if cookies:
                self._get_client().cookies.update(cookies)
    ...
    def warmup_connection(self) -> None:
        try:
            self._get_client().head("https://show.bilibili.com", timeout=5)
            logger.debug("HTTP/2 connection warmed up")
        except Exception:
            pass
